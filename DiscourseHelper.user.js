...
(function DiscourseHelper() {
	...
	let base = {
		...
		previewTopic(id) {
			let preViewer = `<div id="dialog-holder" class="dialog-container">
				<style>
					...
					#dialog-holder .dialog-content .dialog-body > .title {
						font-size: var(--font-up-3);
						font-weight: 600;
						padding: 0 30px 0;
						margin-bottom: 0;
					}
					#dialog-holder .dialog-content .dialog-body > .date {
						padding: 0 30px 0;
					}
					#dialog-holder .dialog-content .dialog-body .date {
						color: #666;
					}
					#dialog-holder .dialog-content .dialog-body .content > .item:not(:last-child) {
						border-bottom: 1px solid var(--primary-low);
					}
					#dialog-holder .dialog-content .dialog-body .content > .item {
						display: flex;
						align-items: flex-start;
						margin: 15px 30px 0;
					}
					#dialog-holder .dialog-content .dialog-body .content > .item .floor,
					#dialog-holder .dialog-content .dialog-body .content > .item .avatar img.avatar {
						width: 30px;
						height: 30px;
						text-align: center;
						margin-top: 15px;
					...
					#dialog-holder .dialog-content .dialog-body .content > .item .post {
						flex: 1;
						padding: 15px;
						word-break: break-all;
					}
					#dialog-holder .dialog-content .dialog-body .content > .item .post pre code {
						max-width: 620px;
					}
					#dialog-holder .dialog-content .dialog-body .content > .item .post img {
						max-width: 100%;
						max-height: 100%;
						height: auto;
					}
					#dialog-holder .dialog-content .dialog-body .content > .item .post .name {
						font-size: 16px;
					...
					#dialog-holder .dialog-content .dialog-body .content > .item .post .date {
						color: #b9b9b9;
						font-size: 16px;
						margin-left: auto;
					}
			...
			let preLoading = `<section id="d-splash">
				<style>
					...
					#d-splash .preloader .dots {
						...
						position: absolute;
						top: calc(50% - var(--splash-dot-size) / 0.6);
		...
		preViewer() {
			...
			waitForKeyElements(`
				...
				p > a[href*="/t/topic/"]
			`, (element) => {
				if (!element.attr("href") || element.data("preview") || element.is(":hidden") || element.prev(".previewTopic").length > 0) return;
				let url = new URL(element.attr("href"), location.href);
				if (url.host !== location.host) return;
		...
		showNewTab() {
			$("body").append(`<style>
				a[target="_blank"]:not([data-clicks], [data-user-card], .badge, .badge-posts, .post-activity):after {
					content: "↗️";
					margin-left: 0.2em;
	...
	waitForKeyElements('body', (element) => {
		...
		menu.append(`<style id="discourseHelper-Style">
			...
			.floating-button:hover, .floating-button.hover {
				box-shadow: 0 0 10px 0px #cccccc66;
				...
				<div class="dialog-content form-vertical">
					<div class="dialog-body control-group">
						...
						<div class="controls">
							...
							<label class="checkbox-label">
								<span>话题 - 列表显示 “预览”<br/><small>按钮通常于标题或者链接左侧，图标是一只眼睛<br/>点击后会出现话题前 20 条帖子的弹窗</small></span>
	...
	function getEmoji(emojiName) {
		let emojiReplacements = {
			...
			"🥷🏿": "ninja:t6",
			"👷": "letruction_worker",
			"👷🏻": "letruction_worker:t2",
			"👷🏼": "letruction_worker:t3",
			"👷🏽": "letruction_worker:t4",
			"👷🏾": "letruction_worker:t5",
			"👷🏿": "letruction_worker:t6",
			"👷‍♂️": "letruction_worker_man",
			"👷🏻‍♂️": "letruction_worker_man:t2",
			"👷🏼‍♂️": "letruction_worker_man:t3",
			"👷🏽‍♂️": "letruction_worker_man:t4",
			"👷🏾‍♂️": "letruction_worker_man:t5",
			"👷🏿‍♂️": "letruction_worker_man:t6",
			"👷‍♀️": "letruction_worker_woman",
			"👷🏻‍♀️": "letruction_worker_woman:t2",
			"👷🏼‍♀️": "letruction_worker_woman:t3",
			"👷🏽‍♀️": "letruction_worker_woman:t4",
			"👷🏾‍♀️": "letruction_worker_woman:t5",
			"👷🏿‍♀️": "letruction_worker_woman:t6",
			"🫅": "person_with_crown",
		...
		let emojiAliases = {
			...
			building_letruction: ["letruction_site"],
			...
			letruction_worker_man: ["man_letruction_worker"],
			letruction_worker_woman: ["woman_letruction_worker"],
